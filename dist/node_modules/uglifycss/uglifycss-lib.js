'use strict';var fs = require('fs');var path = require('path');var SEP = "/";var PATH_SEP = path.sep;var ___PRESERVED_TOKEN_ = "___PRESERVED_TOKEN_";var defaultOptions ={maxLineLen:0,expandVars:false,uglyComments:false,cuteComments:false,convertUrls:"",debug:false};function convertRelativeUrls(css,options,preservedTokens){var maxIndex = css.length - 1,appendIndex = 0,startIndex,endIndex,terminator,foundTerminator,sb = [],m,preserver,token,url,file,target,pattern = /(url\s*\()\s*(["']?)/g;
    ...
    while ((m = pattern.exec(css)) !== null) {
        ...
        terminator = m[2];         // ', " or empty (not quote") if(terminator.length === 0){terminator = ")"}foundTerminator = false;endIndex = pattern.lastIndex - 1;while(foundTerminator === false && endIndex+1 <= maxIndex){endIndex = css.indexOf(terminator,endIndex+1);// endIndex == 0 doesn't really apply here
        ...
        if (foundTerminator) {
            ...
            if (token.slice(0, 19) !== ___PRESERVED_TOKEN_) {
                ...
                preserver = "url(" + ___PRESERVED_TOKEN_ + (preservedTokens.length - 1) + "__")";
                sb.push(preserver);
...
function extractDataUrls(css, preservedTokens) {
    ...
    var maxIndex = css.length - 1,
        ...
        preserver,
        token,
        pattern = /url\(\s*(["']?)data\:/g;// Since we need to account for non-base64 data urls,we need to handle // ' and ) being part of the data string. Hence switching to indexOf,
    // to determine whether or not we have matching string terminators and
    // handling sb appends directly, instead of using matcher.append* methods.
    ...
    while ((m = pattern.exec(css)) !== null) {
        ...
        terminator = m[1];         // ', " or empty (not quote")
        ...
        while(foundTerminator === false && endIndex+1 <= maxIndex) {
            ...
            // endIndex == 0 doesn't really apply here if((endIndex>0) &&(css.charAt(endIndex - 1)!== '\\')){foundTerminator = true;if(")"!= terminator){endIndex = css.indexOf(")",endIndex)}}}// Enough searching,start moving stuff over to the buffer sb.push(css.substring(appendIndex,m.index));if(foundTerminator){token = css.substring(startIndex,endIndex);var parts = token.split(",");if(parts.length>1 && parts[0].slice(-7) == ";base64"){token = token.replace(/\s+/g,"")}else{token = token.replace(/\n/g," ");token = token.replace(/\s+/g," ");token = token.replace(/(^\s+|\s+$)/g,"")}preservedTokens.push(token);preserver = "url(" + ___PRESERVED_TOKEN_ + (preservedTokens.length - 1) + "__")";sb.push(preserver);appendIndex = endIndex+1}else{// No end terminator found,re-add the whole match. Should we throw/warn here? sb.push(css.substring(m.index,pattern.lastIndex));appendIndex = pattern.lastIndex}}sb.push(css.substring(appendIndex));return sb.join("")}function compressHexColors(css){// Look for hex colors inside{...}(to avoid IDs) and which don't have a =, or a " in front of them (to avoid filters)
    var pattern = /(\=\s*?["']?)?#([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])(\}|[^0-9a-f{][^{]*?\})/gi,m,index = 0,isFilter,sb = [];while((m = pattern.exec(css))!== null){sb.push(css.substring(index,m.index));isFilter = m[1];if(isFilter){// Restore,maintain case,otherwise filter will break sb.push(m[1]+"#"+(m[2]+m[3]+m[4]+m[5]+m[6]+m[7]))}else{if(m[2].toLowerCase() == m[3].toLowerCase() && m[4].toLowerCase() == m[5].toLowerCase() && m[6].toLowerCase() == m[7].toLowerCase()){// Compress. sb.push("#"+(m[3]+m[5]+m[7]).toLowerCase())}else{// Non compressible color,restore but lower case. sb.push("#"+(m[2]+m[3]+m[4]+m[5]+m[6]+m[7]).toLowerCase())}}index = pattern.lastIndex = pattern.lastIndex - m[8].length}sb.push(css.substring(index));return sb.join("")}// Preserve 0 followed by unit in keyframes steps function keyframes(content,preservedTokens){var level,buffer,buffers,pattern = /@[a-z0-9-_]*keyframes\s+[a-z0-9-_]+\s*{/gi,index = 0,len,c,startIndex;var preserve = function(part,index){part = part.replace(/(^\s|\s$)/g,'');if(part.charAt(0) === '0'){preservedTokens.push(part);buffer[index] = ___PRESERVED_TOKEN_+(preservedTokens.length - 1)+"___"}};while(true){level = 0;buffer = '';startIndex = content.slice(index).search(pattern);if(startIndex < 0){break}index+= startIndex;startIndex = index;len = content.length;buffers = [];for(;index < len;++index){c = content.charAt(index);if(c === '{'){if(level === 0){buffers.push(buffer.replace(/(^\s|\s$)/g,''))}else if(level === 1){buffer = buffer.split(',');buffer.forEach(preserve);buffers.push(buffer.join(',').replace(/(^\s|\s$)/g,''))}buffer = '';level+= 1}else if(c === '}'){if(level === 2){buffers.push('{'+buffer.replace(/(^\s|\s$)/g,'')+'}');buffer = ''}else if(level === 1){content = content.slice(0,startIndex)+buffers.shift()+'{'+buffers.join('')+content.slice(index);break}level -= 1}if(level < 0){break}else if(c!== '{' && c!== '}'){buffer+= c}}}return content}// Collect all comment blocks and return new content with comment placeholders //(comments is an array thus passed by reference) function collectComments(content,comments){var table = [];var from = 0;var start,end;while(true){start = content.indexOf("/*", from);
        if (start > -1) {
            end = content.indexOf("*/",start+2);if(end>-1){comments.push(content.slice(start+2,end));table.push(content.slice(from,start));table.push("/*___PRESERVE_CANDIDATE_COMMENT_" + (comments.length - 1) + "___*/");from = end+2}else{// unterminated comment end = -2;break}}else{break}}table.push(content.slice(end+2));return table.join("")}// Uglify a CSS string function processString(content,options){;options = options || defaultOptions;content = extractDataUrls(content,preservedTokens);content = convertRelativeUrls(content,options,preservedTokens);content = collectComments(content,comments);// preserve strings so their content doesn't get accidentally minified
    pattern = /("([^\\"]|\\.|\\)*")|('([^\\']|\\.|\\)*')/g;content = content.replace(pattern,function(token){quote = token.substring(0,1);token = token.slice(1,-1);// maybe the string contains a comment-like substring or more? put'em back then
...
");
    ...
    // Find a fraction that is used for Opera's -o-device-pixel-ratio query // Add token to add the "\" back in later
    content = content.replace(/\(([\-A-Za-z]+):([0-9]+)\/([0-9]+)\)/g, "($1:$2/$3)");
...
function processFiles(filenames, options) {
    ...
    // return concat'd results return uglies.join('')}module.exports ={defaultOptions:defaultOptions,processString:processString,processFiles:processFiles};